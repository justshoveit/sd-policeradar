...
function savePositions() {
	...
	fetch('https://ornate_policeradar/savePositions', {
		method: 'POST',
		headers: {
...
function init() {
	...
	radarPanel.style.left = `${Math.max(
		0,
		(window.innerWidth - radarWidth) / 2
	)}px`;
	radarPanel.style.top = `${Math.max(
		0,
		(window.innerHeight - radarHeight) / 2
	)}px`;
	radarPanel.style.transform = 'none';
	logPanel.style.left = `${Math.max(
		0,
		(window.innerWidth - radarWidth) / 2 - logWidth - 20
	)}px`;
	logPanel.style.top = `${Math.max(
		0,
		(window.innerHeight - radarHeight) / 2
	)}px`;
...
function setupEventListeners() {
	...
	boloPlateInput.addEventListener('focus', () => {
		fetch('https://ornate_policeradar/inputActive', {
			method: 'POST',
			headers: {
				'Content-Type': 'application/json',
			},
	...
	boloPlateInput.addEventListener('blur', () => {
		fetch('https://ornate_policeradar/inputInactive', {
			method: 'POST',
			headers: {
				'Content-Type': 'application/json',
			},
...
function checkBoloMatch(direction, plate) {
	...
	const isMatch = state.boloPlates.some(
		(boloPlate) => boloPlate.trim().toUpperCase() === cleanPlate
	);
	if (isMatch) {
		if (direction === 'front') {
			if (!state.boloAlerts.front) {
				...
				fetch('https://ornate_policeradar/boloAlert', {
					method: 'POST',
					headers: {
						'Content-Type': 'application/json',
					},
...
function addBoloPlate() {
	...
	if (plate) {
		...
		fetch('https://ornate_policeradar/addBoloPlate', {
			method: 'POST',
			headers: {
				'Content-Type': 'application/json',
			},
...
function removeBoloPlate(plate) {
	...
	fetch('https://ornate_policeradar/removeBoloPlate', {
		method: 'POST',
		headers: {
			'Content-Type': 'application/json',
		},
...
function saveReading() {
	...
	const newReading = {
		...
		timestamp,
		speed:
			state.selectedDirection === 'Front' ? state.frontSpeed : state.rearSpeed,
		plate:
			state.selectedDirection === 'Front' ? state.frontPlate : state.rearPlate,
...
function deleteReading(id) {
	state.savedReadings = state.savedReadings.filter(
...
function showNotification(message) {
	if (state.notificationType === 'native') {
		...
		fetch('https://ornate_policeradar/showNotification', {
			method: 'POST',
			headers: {
				'Content-Type': 'application/json',
			},
